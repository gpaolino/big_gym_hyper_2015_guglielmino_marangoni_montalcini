        ...
        success: function(response) {
            ...
            while (insertedCC < courseCategories.length) {
                    ...
                    if(courseCategories[i].active == 1) {
                       el+="<div class='col-sm-3' style='margin-top: 10px'> <h4>"+courseCategories[i].full_name+"</h4> <img class='media-object rounded-img' src='"+courseCategories[i].image+"' alt='Generic placeholder image'></div> <div class='col-sm-3' style='margin-top: 45px; margin-bottom: 35px'>  <li><a href='courseCategory.html?par="+courseCategories[i].full_name+"'>What is "+courseCategories[i].full_name+"?<span class='sr-only'>(current)</span></a></li> <li><a href='coursesByCourseCategory.html?par="+courseCategories[i].full_name+"&par2=allCourseCategories'>Courses for "+courseCategories[i].full_name+"<span class='sr-only'>(current)</span></a></li></div>";
                    } else {
                        el+="<div class='col-sm-3' style='margin-top: 10px'> <h4>"+courseCategories[i].full_name+"</h4> <img class='media-object rounded-img' src='"+courseCategories[i].image+"' alt='Generic placeholder image'></div> <div class='col-sm-3' style='margin-top: 45px; margin-bottom: 35px'>  <li><a class='inactiveLink' href='#'>What is "+courseCategories[i].full_name+"?<span class='sr-only'>(current)</span></a></li> <li><a class='inactiveLink' href='#'>Courses for "+courseCategories[i].full_name+"<span class='sr-only'>(current)</span></a></li></div>";
                    }
                    insertedCC++; 
                    first=1; 
                } else {
                    if(courseCategories[i+offset].active == 1) {
                        el+="<div class='col-sm-3' style='margin-top: 10px'> <h4>"+courseCategories[i+offset].full_name+"</h4> <img class='media-object rounded-img' src='"+courseCategories[i+offset].image+"' alt='Generic placeholder image'></div> <div class='col-sm-3' style='margin-top: 45px; margin-bottom: 35px'>  <li><a href='courseCategory.html?par="+courseCategories[i+offset].full_name+"'>What is "+courseCategories[i+offset].full_name+"?<span class='sr-only'>(current)</span></a></li> <li><a href='coursesByCourseCategory.html?par="+courseCategories[i+offset].full_name+"&par2=allCourseCategories'>Courses for "+courseCategories[i+offset].full_name+"<span class='sr-only'>(current)</span></a></li></div>";
                    } else {
